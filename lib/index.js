...
function buildSuggestionConfig({ instance, instanceData, tippy, triggerChar }) {
    if (typeof tippy !== 'function') {
        console.error(
            '[tiptapMentionCore] window.tiptap.tippy is not a function. ' +
            'The mention popup will not open. Verify that dist.js (main bundle) ' +
            'exports tippy and is loaded before the lib bundle.'
        );
        // Degrade gracefully: suggestion extension still loads, popup just won't appear.
        return { char: triggerChar, items: () => [], render: () => ({}) };
    }
